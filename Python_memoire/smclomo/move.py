...
def Gibbs_move(n_moves, step_size, log_prob, log_prob_prior, init_parms, γ, d):   
    ...
    for noise in np.atleast_2d(epsilon):
        parms_perturbed = trace[-1] + noise
        parms_new = np.array(trace[-1], dtype=float)
        accepted = []
        for i in range(d):
            

            np.put(parms_new, i, parms_perturbed[i])
            # acceptance rate
        ...
        trace =  np.vstack((trace, parms_new))
        accepted_matrix = np.vstack((accepted_matrix, accepted))
